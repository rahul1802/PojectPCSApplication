...
public class EmpSkillExec {
	...
	{
		List<EmpSkill> skillList=skillController.getAllEmployeesSkill();
	...
	public void getEmpSkillById()
	{
		...
		EmpSkill s=skillController.getEmployeeSkillByID(id);
		System.out.println(s);
	}
...
public void addEmpSkill()
{
	...
	skillController.addEmployeeSkill(s);
}
public void updateEmpSkill() {
	try {
		...
		id=Integer.parseInt(reader.readLine());
		EmpSkill skill=skillController.getEmployeeSkillByID(id);
		...
		if(exp==(confirmexp)) {
			...
			skill.setEsId(id);
			skillController.updateEmployeeSkill(skill);
...
public void deleteEmpSkill() {
	try {
		...
		id=Integer.parseInt(reader.readLine());
		skillController.deleteEmployeeSkill(id);
	}
	catch(IOException ex) {
		System.out.println(ex.getMessage());
	}
