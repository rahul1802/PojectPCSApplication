...
public class EmpJobExec {
	...
	{
		List<EmpJob> jobList=jobController.getAllEmpJob();
	...
	public void getEmpJobById()
	{
		...
		EmpJob s=jobController.getEmpJobByID(id);
		System.out.println(s);
	}
...
public void updateEmpJob() {
	try {
		...
		id=Integer.parseInt(reader.readLine());
		EmpJob job=jobController.getEmpJobByID(id);
		...
		if(rec.equals(confirmrec)) {
			...
			job.setEjId(id);
			jobController.updateEmpJob(job);
